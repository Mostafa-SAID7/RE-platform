      ...
      <div class="relative">
        ...
        <div class="flex items-center gap-2">
          <input
            ...
            readonly
            [attr.aria-label]="label || 'Date range picker'"
          ...
          <button
            ...
            [attr.aria-label]="'Open date range picker'"
          >
        ...
        <!-- Calendar Popup -->
        <div *ngIf="isOpen()" class="absolute top-full left-0 mt-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg shadow-lg dark:shadow-xl p-4 z-50 w-96">
          <!-- Header -->
          ...
          <div class="grid grid-cols-7 gap-1 mb-4">
            <button
              *ngFor="let day of calendarDays()"
              ...
              [class.bg-blue-600]="isStartDate(day)"
              [class.bg-blue-100]="isInRange(day) && !isStartDate(day) && !isEndDate(day)"
              [class.dark:bg-blue-900]="isInRange(day) && !isStartDate(day) && !isEndDate(day)"
              [class.bg-blue-600]="isEndDate(day)"
              [class.text-white]="isStartDate(day) || isEndDate(day)"
              [class.text-blue-600]="isInRange(day) && !isStartDate(day) && !isEndDate(day)"
              [class.dark:text-blue-300]="isInRange(day) && !isStartDate(day) && !isEndDate(day)"
              [class.text-gray-400]="day === null"
              [class.text-gray-600]="day !== null && !isInRange(day)"
              [class.dark:text-gray-400]="day === null"
              [class.dark:text-gray-300]="day !== null && !isInRange(day)"
              [class.hover:bg-blue-700]="isStartDate(day) || isEndDate(day)"
              [class.hover:bg-blue-200]="isInRange(day) && !isStartDate(day) && !isEndDate(day)"
              [class.dark:hover:bg-blue-800]="isInRange(day) && !isStartDate(day) && !isEndDate(day)"
              [class.hover:bg-gray-100]="day !== null && !isInRange(day)"
              [class.dark:hover:bg-gray-800]="day !== null && !isInRange(day)"
...
export class DateRangePickerComponent implements ControlValueAccessor {
  @Input() id: string = 'date-range-picker-' + Math.random().toString(36).substring(2, 11);
  @Input() label: string = '';
  ...
  private onChangeFn: (value: DateRange) => void = () => {};
  ...
  isStartDate(day: number | null): boolean {
    if (day === null) return false;
    const date = new Date(this.currentMonth());
    date.setDate(day);
    const dateStr = date.toISOString().split('T')[0] || '';
    return dateStr === this.tempStartDate();
  }

  isEndDate(day: number | null): boolean {
    if (day === null) return false;
    const date = new Date(this.currentMonth());
    date.setDate(day);
    const dateStr = date.toISOString().split('T')[0] || '';
    return dateStr === this.tempEndDate();
  }
